...
# Apply a divergent colormap with a narrower range for higher color contrast
color_map = 'seismic'
vmin, vmax = -0.02, 0.02  # Narrower range for more pronounced color changes

# Corner indices for pinning
corner_indices = [
    (0, 0, 0), (0, 0, Nz - 1), (0, Ny - 1, 0), (0, Ny - 1, Nz - 1),
    (Nx - 1, 0, 0), (Nx - 1, 0, Nz - 1), (Nx - 1, Ny - 1, 0), (Nx - 1, Ny - 1, Nz - 1)
]
...
def update(frame):
    ...
    # Time-stepping finite difference calculation in 3D with internal coupling along z
    ...
    # Fix displacements at corners to zero to keep them stationary
...
ani.save("membrane_simulation_box_thickness_fixed_corners_enhanced_colors.gif", fps=30)
